...
const updateOnMountAndKeysChange = (
  ...
  compose(
    ...
    lifecycle({
      componentWillReceiveProps(nextProps) {
        ...
        if (shouldComponentUpdate) {
          ...
          setIsLoading(true);
          R.composeP(() => setIsLoading(false), update)(nextProps).catch(() =>
            setIsLoading(false),
          );
        }
      },
      componentDidMount() {
        const { setIsLoading } = this.props;
        setIsLoading(true);
        R.composeP(() => setIsLoading(false), update)(this.props).catch(() =>
          setIsLoading(false),
        );
      },
    }),
    ...
    WrappedComponent => ({ isLoading, ...rest }) =>
      <div
        style={{ opacity: isLoading && shouldDisplayLoadingState ? 0.7 : 1 }}
      >
        <WrappedComponent {...{ isLoading, ...rest }} />
      </div>,
  );
